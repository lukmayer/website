...
Feature parity also implies that there should be an internal page version, a panel version and a pop-out version of every major feature. This is not the case for most features! I think Downloads is the only one to have all three versions close to parity. 

\

## The unwritten rules of UI customization

Vivaldi lets you add/remove buttons from the UI, as well as move them around. You can also hide/unhide the bars on which these buttons sit. Basically there is the Window Title bar, the Tab bar, the Address bar, and the Status bar. There are quite a few non-obvious rules about this customization. For example, you can put icons (e.g. panels) on the status bar, hide the status bar, and still use the panel so long as you hit the keyboard shortcut to activate it! This is not true for the panel bar, which is usually the home of these buttons. Every time you hide the panel bar, the panel will unload, which means opening panels takes a lot longer than it should. If you activate a panel while the panel bar is hidden, it will become unhidden even if the panel button is not actually on the panel bar. Personally, I find this automatic unhiding of the panel bar and its associated unloading behavior pretty annoying, and I wish this could be changed.
